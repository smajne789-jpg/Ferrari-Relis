...
async def check_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"🎁 Чек создан\n\nhttps://t.me/{bot_info.username}?start=check_{code}"
    )
    await state.clear()
...
async def create_dep_check(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return

    await state.set_state(CreateDepositCheck.waiting_activations)
    await callback.message.answer("Введите количество активаций")

@dp.message(CreateDepositCheck.waiting_activations)
async def dep_check_activations(message: Message, state: FSMContext):
    await state.update_data(activations=int(message.text))
    await state.set_state(CreateDepositCheck.waiting_amount)
    await message.answer("Введите сумму за активацию")

@dp.message(CreateDepositCheck.waiting_amount)
async def dep_check_amount(message: Message, state: FSMContext):
    await state.update_data(amount=float(message.text))
    await state.set_state(CreateDepositCheck.waiting_deposit)
    await message.answer("Введите необходимую сумму депозитов")

@dp.message(CreateDepositCheck.waiting_deposit)
async def dep_check_deposit(message: Message, state: FSMContext):
    data = await state.get_data()
    ...
    await message.answer(
        f"💎 Депозитный чек создан\n\n"
        f"🔗 https://t.me/{bot_info.username}?start=check_{code}\n\n"
        f"💰 Требуется депозит: {float(message.text)}$"
    )
